    ...
    return (
        ...
        <div className="flex items-center justify-center">
            <form onSubmit={handleSignin} method="POST" className="w-96 h-80 p-5 my-40 bg-black text-white flex flex-col rounded-md shadow-lg shadow-neutral-900">
                ...
                <input onChange={(input) => setEmail(input.target.value)} className="m-3 px-4 py-2 rounded-md bg-slate-600 hover:bg-slate-700 transition delay-75 hover:outline-cyan-500 outline-none" type="email" name="Email" placeholder="Email"/>
                <input onChange={(input) => setPassword(input.target.value)} className="m-3 px-4 py-2 rounded-md bg-slate-600 hover:bg-slate-700 transition delay-75 hover:outline-cyan-500 outline-none" type="password" name="Password" placeholder="Password"/>
